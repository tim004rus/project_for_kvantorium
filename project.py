...
def rad1():
	...
	def clicked():
		...
		if probability == 1:
			res = "Доклад про {}.\nТеперь и с перцем.".format(txt.get())
			lbl.configure(text=res)
		if probability == 2:
			res = "Сочинение о {}.\nВозможна поломка мозгов.".format(txt.get())
			lbl.configure(text=res)
		if probability ==3:
			res = "Доклад о {}.\nНе открывать до 30.07.30.".format(txt.get())
			lbl.configure(text=res)
		if probability == 4:
			res = "Сочинение о {}.\nВсе права защищены.".format(txt.get())
			lbl.configure(text=res)
...
def rad2():
	...
	def clicked():
		...
		if probability == 1:
			res = "Творческая работа по теме {}.\nКто нашёл тот здохнет".format(txt.get())
			lbl.configure(text=res)
		if probability == 2:
			res = "Творческая работа по теме {}.\nС любовью из психушки.".format(txt.get())
			lbl.configure(text=res)
		if probability ==3:
			res = "Творческая работа по теме {}.\nДальтоникам посвящается...".format(txt.get())
			lbl.configure(text=res)
		if probability == 4:
			res = "Творческая работа по теме {}.\nПотом сделаю, честно".format(txt.get())
			lbl.configure(text=res)
...
def rad3():
	...
	def clicked():
		...
		if probability == 1:
			res = "Каталог {}.\nОн точно был где-то тут.".format(txt.get())
			lbl.configure(text=res)
		if probability == 2:
			res = "Сборник {}.\nМы проверили, всё чисто.".format(txt.get())
			lbl.configure(text=res)
		if probability ==3:
			res = "Каталог {}.\nКажется, вирусняк.".format(txt.get())
			lbl.configure(text=res)
		if probability == 4:
			res = "Сборник {}.\nАбсолютно бесполезен.".format(txt.get())
			lbl.configure(text=res)
...
def rad4():
	...
	def clicked():
		...
		if probability == 1:
			res = "{}.\nВход в даркнет рядом.".format(txt.get())
			lbl.configure(text=res)
		if probability == 2:
			res = "{}.\nЗанято!".format(txt.get())
			lbl.configure(text=res)
		if probability ==3:
			res = "{}.\nОпять заразу из интернета тащит.".format(txt.get())
			lbl.configure(text=res)
		if probability == 4:
			res = "{}.\nЭто не я, клянусь.".format(txt.get())
			lbl.configure(text=res)
